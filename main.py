...
# ── Auth ─────────────────────────────────────────────────────────────────────
AUTH = {"password": os.environ.get("ADMIN_PASSWORD", "admin")}
SESSIONS: dict = {}
...
async def startup():
    global http_client
    load_data()  # 1. Load your saved items first
    limits = httpx.Limits(max_connections=500, max_keepalive_connections=100)
    timeout = httpx.Timeout(30.0, connect=10.0)
    http_client = httpx.AsyncClient(limits=limits, timeout=timeout, follow_redirects=True)
    asyncio.create_task(keep_alive())
    asyncio.create_task(periodic_save_task())  # 2. Start the auto-saver loop
    await ensure_default_link()

@app.on_event("shutdown")
async def shutdown():
    logger.info("Server shutting down. Forcing final save to db.json...")
    await save_data_to_disk() # Forces a final save before turning off!
    if http_client:
    ...
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, "r", encoding="utf-8") as f:
                ...
                AUTH.update(data.get("auth", {}))
                
                # Load saved global traffic AND requests
...
async def save_data_to_disk():
    try:
        async with LINKS_LOCK:
            links_copy = json.loads(json.dumps(LINKS))
        async with CUSTOM_ADDRESSES_LOCK:
            addresses_copy = list(CUSTOM_ADDRESSES)
        
        data = {
            "links": links_copy,
            "custom_addresses": addresses_copy,
            "auth": dict(AUTH),
            "stats": {
                "total_bytes": stats["total_bytes"],
                "total_requests": stats["total_requests"] # Save total requests here
            }
        }
...
async def api_login(request: Request):
    ...
    if password != AUTH.get("password", "admin"):
...
async def api_change_password(request: Request, _=Depends(require_auth)):
    ...
    if current != AUTH.get("password", "admin"):
...
async def websocket_tunnel(websocket: WebSocket, uuid: str):
    ...
    try:
        ...
        # Saves the request to db.json instantly!
...
# ── HTML ──────────────────────────────────────────────────────────────────────
# ── CAMOUFLAGE DECOY HTML ─────────────────────────────────────────────────────
DECOY_HTML = r"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Johnson Connect</title>
    <link rel="icon" href="https://i.imgur.com/JBas06d.jpeg" type="image/jpeg">
    <style>
        body { 
            margin: 0; 
            background: #060608; 
        }
        /* The container keeps the image and the secret button locked together */
        .page-container {
            position: relative;
            width: 100%;
            max-width: 1400px; /* Prevents it from getting absurdly huge on wide monitors */
            margin: 0 auto;    /* Centers the layout */
            display: block;
        }
        /* Using a real image tag fixes the cut-off scrolling issue instantly */
        .hero-img { 
            width: 100%; 
            height: auto;
            display: block; 
        }
        /* The invisible clickable box sitting directly over the AI Models button */
        .secret-btn {
            position: absolute;
            top: 30%;    /* Vertical placement over AI Models button */
            left: 42.5%;   /* Horizontal placement */
            width: 15%;    /* Matches button width */
            height: 1.6%;  /* Matches button height */
            cursor: pointer;
            z-index: 10;
            
            /* PRO-TIP: Uncomment the line below to see a red box. 
               Use it to align exactly, then delete the line to make it invisible again! */
...
<body>
    <div class="page-container">
        <img src="https://i.imgur.com/26QWrU6.png" class="hero-img" alt="Johnson Connect">
        
        <!-- The secret AI Models button overlay -->
        <div class="secret-btn" onclick="promptSecret()"></div>
    </div>

    <script>
        // When clicked, asks for a code. Entering 55 redirects to the login page.
        function promptSecret() {
            let code = prompt("System Access Code:");
            if (code === "55") {
